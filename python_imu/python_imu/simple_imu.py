# coding:UTF-8
# Version: V1.5.1
# python should be in your system 
# install serial
...
start = 0 #frame header start
data_length = 0 #document length is 11 eg:55 51 31 FF 53 02 CD 07 12 0A 1B
...
def GetDataDeal(list_buf):
    ...
    if(list_buf[buf_length - 1] != CheckSum): #invalid checksum
        return
        
    if(list_buf[1] == 0x51): #acceleration output
        for i in range(6): 
            ACCData[i] = list_buf[2+i] #valid data assingment
        acc = get_acc(ACCData)

    elif(list_buf[1] == 0x52): #angular vel output
        for i in range(6): 
            GYROData[i] = list_buf[2+i] #valid data
        gyro = get_gyro(GYROData)

    elif(list_buf[1] == 0x53): #angle 
        for i in range(6): 
            AngleData[i] = list_buf[2+i] #valid data
...
def DueData(inputdata):  # New core procedures, read the data partition, each read to the corresponding array 
    ...
    if inputdata == 0x55 and start == 0:   ## port status and start == 0 ()
        start = 1
        data_length = 11
        CheckSum = 0
        #清0
    ...
    if start == 1:    ## start == 1 ()
        CheckSum += inputdata # Checksum calculation will add the checksum bit.
...
if __name__ == '__main__':
    ...
    while(1):
        ...
        RXdata = int(RXdata.hex(),16) #convert to hex
        DueData(RXdata)
